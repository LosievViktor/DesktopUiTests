...
public sealed class CollectionTests : UiTestBase
{
	...
	public void CollectionsPage_NewTodoEntry_IsDisplayed()
	{
		...
		Assert.That(page.NewTodoEntry, Is.Not.Null, "Check that new todo entry is displayed");
	}
	...
	public void CollectionsPage_AddButton_IsEnabledWithEnglishText()
	{
		...
		Assert.That(page.AddButton.IsEnabled, Is.True, "Check that Add button is enabled");
		Assert.That(page.AddButton.Name, Does.Contain(EnglishTexts.Collections.Add), "Check that Add button shows English text");
	}
	...
	public void CollectionsPage_TodoList_ShowsSeededEnglishItems()
	{
		...
		Assert.That(page.TodoList, Is.Not.Null, "Check that todo list is displayed");
		Assert.That(page.HasTodoWithTitle(EnglishTexts.Collections.Todo1), Is.True, "Check that seeded English todo item is present");
		Assert.That(page.GetTodoDoneCheckboxes().Length, Is.GreaterThanOrEqualTo(1), "Check that at least one todo done checkbox is present");
	}
	...
	public void CollectionsPage_AddTodo_InsertsNewItem()
	{
		...
		Assert.That(page.HasTodoWithTitle(title), Is.True, "Check that newly added todo item is present in the list");
		Assert.That(page.IsEmptyLabelVisible(), Is.False, "Check that empty list label is not visible after adding a todo");
	}
	...
	public void CollectionsPage_TodoDoneCheckbox_CanToggle()
	{
		...
		Assert.That(after, Is.EqualTo(!initial), "Check that todo done checkbox toggles its state");
	}
}
